...
blacklist_str = {"8.247.", "8.248.", "78.40.", "35.186."}  # If no blacklist string then {}    # string is compared to log_str variable
        ...
        if not (ip_dst in dns_table):
            if resolve_dns and not (ip_dst.startswith('192.168.')):
                ...
                try:
                    ...
                    # answers = dns.resolver.query(no, 'PTR')
                    host = socket.gethostbyaddr(ip_dst)

                    # my_resolver = dns.resolver.Resolver()
                    # answers = my_resolver.resolve(ip_dst, "PTR")
                    # answers_txt = my_resolver.resolve(ip_dst, "TXT")
            ...
            for str_to_search in log_only_str:
                if str_to_search in log_str:
                    if LOG_TO_SCREEN:
                        ...
                        # t = Thread(target=log_to_screen, args=(ip_src,))
                        # t.start()
